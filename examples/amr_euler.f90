...
program euler
  ...
  do 
   ...
   dt = 0.2_dp/(sum(wSp/dr_min) + epsilon(1.0_dp))
    if (mod(t_iter, 10) == 0) then
      write(fname, "(A,I0)") "amr_euler_" // DIMNAME // "_", t_iter
  ...
  subroutine ref_rout( box, cell_flags )
    ...
    nc = box%n_cell
    do KJI_DO(1,nc)
      diff = box%dr(1)**2*abs(box%cc(i+1,j,ic1)+box%cc(i-1,j,ic1)-2*box%cc(i,j,ic1)) + &
              box%dr(2)**2*abs(box%cc(i,j+1,ic1)+box%cc(i,j-1,ic1)-2*box%cc(i,j,ic1))
      if (diff > 1.0e-6_dp .and. box%lvl .le. 5) then
        cell_flags(IJK) = af_do_ref
      else if (diff < 0.1_dp*1.0e-6_dp) then
